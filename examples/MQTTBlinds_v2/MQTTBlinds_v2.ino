...
class MyAM43Callbacks: public AM43Callbacks {
  public:
    ...
    void onPosition(uint8_t pos) {
      ...
      this->mqtt->publish(topic("position").c_str(), String(pos).c_str());
      MQTTclient.publish(topic("position").c_str(), String(pos).c_str());
    }
    ...
    void onBatteryLevel(uint8_t level) {
      ...
      if (this->lastBatteryMessage == 0 || millis() - this->lastBatteryMessage > 30000)  {
        ...
        //this->mqtt->publish(topic("battery").c_str(), String(level).c_str());
        MQTTclient.publish(topic("battery").c_str(), String(level).c_str());
        //Serial.printf("\nDEBUG [%s/%s]\n", topic("battery").c_str(), String(level).c_str());
    ...
    void onLightLevel(uint8_t level) {
      ...
      this->mqtt->publish(topic("light").c_str(), String(level).c_str());
      MQTTclient.publish(topic("light").c_str(), String(level).c_str());
    }
    ...
    void onConnect(AM43Client *c) {
      ...
      this->mqtt->begin(mqtt_server, mqtt_server_port, this->wifiClient);
      this->mqtt->onMessage(mqtt_callback);
...
//      MQTTclient.publish(topic("available").c_str(), "online", true);
      this->mqtt->publish(topic("available").c_str(), "online", true);
      this->mqtt->subscribe(topic("set").c_str(),1);
      this->mqtt->subscribe(topic("set_position").c_str(),1);
      this->mqtt->subscribe(topic("status").c_str(),1);
...
void mqtt_callback(String &topic, String &payload) {
  ...
  if (address == "enable") {
    if (payload == "off") {
      ...
      bleEnabled = false;
      MQTTclient.publish(topPrefix("/enabled").c_str(), "OFF", true);
#ifdef AM43_ONDEMAND
      bleOnDemand = true;
#endif
      for (auto const& c : cls) {
      ...
      bleEnabled = true;
      MQTTclient.publish(topPrefix("/enabled").c_str(), "ON", true);
      lastScan = 0;
#ifdef AM43_ONDEMAND
      bleOnDemand = false;
#endif
...
void reconnect_mqtt() {
  if (WiFi.status() == WL_CONNECTED && millis() > nextMqttAttempt) {
    ...
    if (!MQTTclient.connected()) {
      ...
      MQTTclient.publish(discTopic, discPayload, true);
      String msgEnabled = "ON";
      if (!bleEnabled) msgEnabled = "OFF";
      MQTTclient.publish(topPrefix("/enabled").c_str(), msgEnabled.c_str(), true);
    } else {
...
#endif
  setup_wifi();
  MQTTclient.begin(mqtt_server, mqtt_server_port, espClient);
